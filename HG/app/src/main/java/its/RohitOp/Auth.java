...
public class Auth extends AsyncTask<String, Void, String> {
	...
	public static String Expiracao;
    public native String Title();
    private  String URLSERVER = "https://tmsalmanyt.xyz/vip/login-apk.php";
    ...
    public String doInBackground(String... strings) {
        ...
        try {
            ...
            JSONObject data = new JSONObject();
            data.put(e("756E616D65"), strings[0]);
            data.put(e("70617373"),strings[1]);
    ...
    public void onPostExecute(String s) {
        ...
        if (activity != null) {
            ...
            if (s == null || s.isEmpty()) {
                Toast.makeText(activity, "Error Server....", 1).show();
            ...
            try {
                ...
                if(data.get("Status").toString().equals("Success")) {
                    ...
                    //    getActivity().startService(new Intent(activity, Floater.class));
                    try {
                        Toast.makeText(activity,Html.fromHtml("<b><font color='#00ff00'>Conectado! " + data.get("Dias").toString() + " Dias" + "</font>"), Toast.LENGTH_LONG).show();
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                    
                    Vendedor = data.get("Vendedor").toString();
                    Validade = data.get("Dias").toString();    
                    Expiracao = data.get("SubscriptionLeft").toString();

                    if (data.get("Status").toString().equals("Success")) {
                    }
                } else {
                    Toast.makeText(activity,Html.fromHtml("<font color='#ff0000'>" + data.get("MessageString").toString() + "</font>"), 1).show();
                }
            } catch (Exception printStackTrace) {
            }
        }
    ...
    public static String e(String hex) {
